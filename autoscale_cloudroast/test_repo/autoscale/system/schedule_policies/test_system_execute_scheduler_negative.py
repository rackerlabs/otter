...
class ExecuteNegativeSchedulerPolicy(AutoscaleFixture):
    ...
    def test_system_execute_at_style_scale_up_when_min_maxentities_are_met(self):
        ...
        style scheduler policy to scale up or scale down will not be
        triggered.
        """
    ...
    def test_system_scheduler_down(self):
        """
        Stop the scheduler. Create at style and cron style schedule (every n
        seconds) and verify events accumulate in scaling schedule table.
    ...
    def test_create_multiple_scheduler_policies_to_execute_simaltaneously(self):
        ...
        all of them are triggered by the scheduler, at the same time, and
        ensure all the policies are executed successfully.
