...
class GetMaxManifest(ScalingGroupFixture):
    ...
    @unittest.skip("Hits rate limit causing this test to fail")
    @tags(stress='true')
    ...
    def test_manifest_max_batch(self):
        ...
        policy_resp = self.autoscale_behaviors.create_policy_min_batch(
            self.group.id, sp_name='batch_policy', batch_size=test_num)
        for p in policy_resp:
            ...
            for w in range(test_num):
                ...
                webhook_req_list.append({'name': w_name_num, 'metadata': {'notes': w}})
