...
class ScheduleScalingPolicyNegative(AutoscaleFixture):
    ...
    def test_schedule_at_style_policy_with_different_date_format(self):
        """
        Creating a scaling policy of type schedule with (at style) with different date
    ...
    #     ** fails with 201 **
    #     """
    #     schedule_value = self.autoscale_behaviors.get_time_in_utc(-600)
    #     schedule_policy_at_style = self.autoscale_behaviors.create_schedule_policy_given(
    #         group_id=self.group.id,
    #         sp_change=self.sp_change,
    #         schedule_at=schedule_value)
